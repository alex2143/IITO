...
foreach($xml->room as $room) {
	...
	echo '<polygon points="'.$room->points.'" id="'.$room->id.'poly" onMouseOver="tooltipOn(\''.$room->id.'\')" onMouseOut="tooltipOff(\''.$room->id.'\')" class="roomPoly" />';
}
...
foreach($xml->room as $room) {
	...
	echo '<text id="'.$room->id.'text" x='.$x.' y='.$y.' onMouseOver="tooltipOn(\''.$room->id.'\')" onMouseOut="tooltipOff(\''.$room->id.'\')" class="roomText" >'.
	'<tspan id="'.$room->id.'name" x='.$x.'>'.$room->id.'</tspan>'.
	'<tspan id="'.$room->id.'curr" x='.$x.' dy="24" class="occupationdigit">x</tspan>'.
